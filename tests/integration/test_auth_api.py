...
async def test_change_password(client: AsyncClient) -> None:
    ...
    admin_token = await login(client, settings.bootstrap_admin_email, settings.bootstrap_admin_password)
    client.headers.update({"Authorization": "Bearer " + admin_token})

    email = "password-test-" + uuid4().hex + "@example.com"
    ...
    user_token = await login(client, email, old_password)
    client.headers.update({"Authorization": "Bearer " + user_token})
