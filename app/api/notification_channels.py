...
def list_channels(
    ...
    return [
        _to_response(channel)
        for channel in db.scalars(select(NotificationChannel).order_by(NotificationChannel.name))
    ]
...
def create_channel(
    ...
    if db.scalar(select(NotificationChannel).where(NotificationChannel.name == payload.name)):
        raise HTTPException(
            ...
            detail="A notification channel with this name already exists",
        )
...
def list_deliveries(
    ...
    delivery_status: str | None = Query(
        default=None,
        alias="status",
        pattern=r"^(pending|delivered|failed)$",
    ),
